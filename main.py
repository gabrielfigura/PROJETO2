...
SIGNAL_DELAY_MIN = 5       # Delay mínimo antes de enviar sinal (segundos)
SIGNAL_DELAY_MAX = 12      # Delay máximo antes de enviar sinal (segundos)
...
state: Dict[str, Any] = {
    ...
    "pending_signal_analysis": False,      # Flag: novo resultado detectado, aguardar delay antes de enviar
    "signal_send_after": 0.0,              # Timestamp: quando pode enviar o sinal após delay
}
...
async def try_send_signal():
    ...
    # Verifica se há análise pendente aguardando o delay
    if not state["pending_signal_analysis"]:
        return

    # Ainda não passou o delay inteligente
    if now < state["signal_send_after"]:
        return

    # Delay passou, agora tenta gerar e enviar o sinal
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        while True:
            try:
                ...
                if updated:
                    # 1. Primeiro: validar sinal anterior (se havia um pendente)
                    await resolve_after_result()

                    # 2. Se não está mais esperando resultado, agendar delay para próximo sinal
